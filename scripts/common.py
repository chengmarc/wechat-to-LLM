...
def _decode_ref_content(ref_type: int, ref_content_el: ET.Element | None) -> str:
    ...
    if ref_type == 49:
        # appmsg XML 内部 <type>49</type>（base appmsg，与 local_type 编码无关）；content 是 XML
        if raw_ref.startswith("<?xml") or raw_ref.startswith("<msg"):
            return _parse_share_xml(raw_ref)
        return "[分享]"
...
def _decode_appmsg_xml(xml_text: str, other_table_hash: str | None = None, sender_map: dict | None = None) -> str | None:
    """
    解析 appmsg 消息的 XML，返回人类可读字符串。

    other_table_hash: 对方 wxid 的 MD5（即消息表名去掉 'Msg_' 前缀），
    用于判断引用消息的发送方。双人聊天必传；群聊可不传。
    sender_map: wxid → 显示名映射，群聊下用于解析引用消息发送方。
    """
    try:
    ...
    if atype == 57:
        ...
        ref_chatusr = _el_text(refermsg.find("chatusr"))   # 群聊中实际发送者 wxid
        ref_displayname = _el_text(refermsg.find("displayname"))
        ref_content = _decode_ref_content(ref_type, refermsg.find("content"))

        # 判断被引用消息的发送方
        if other_table_hash and ref_fromusr:
            # 双人聊天：用 fromusr MD5 比对表名
            ref_label = "对方" if hashlib.md5(ref_fromusr.encode()).hexdigest() == other_table_hash else "用户"
        elif sender_map is not None:
            # 群聊：chatusr 是实际发送者 wxid，fromusr 是群本身 id
            lookup_wxid = ref_chatusr or ref_fromusr
            if lookup_wxid:
                ref_label = sender_map.get(lookup_wxid) or ref_displayname or lookup_wxid
            elif ref_displayname:
                ref_label = ref_displayname
            else:
                ref_label = "?"
        else:
            ref_label = "?"
...
def decode_content(
    ...
    if local_type > 100 and local_type % _APPMSG_TYPE_BASE == 49:
        ...
        return _decode_appmsg_xml(xml_str, other_table_hash, sender_map)
...
def db_number(path: str | Path) -> int:
    """从文件名提取库编号用于数值排序，如 message_2.db → 2。"""
    m = re.search(r"message_(\d+)", str(path))
...
def search_contacts(contact_db: str, keyword: str) -> list[tuple]:
    """在 contact.db 中按关键词搜索双人联系人（排除群聊）。"""
...
def search_chatrooms(contact_db: str, keyword: str) -> list[tuple]:
    """在 contact.db 中按关键词搜索群聊。"""
    conn = sqlite3.connect(contact_db)
    kw = f"%{keyword}%"
    rows = conn.execute(
        "SELECT username, nick_name, remark FROM contact "
...
def check_dbs(db_paths: list, table: str, tz: timezone = timezone(timedelta(hours=8))) -> list[tuple]:
    """
    扫描 db_paths，返回含 table 的库列表。
    每项为 (db_path_str, count, min_date_str, max_date_str)。
    """
    results = []
    for db_path in db_paths:
        ...
        if row:
            ...
            s = datetime.fromtimestamp(mn, tz).strftime("%Y-%m-%d") if mn else "?"
            u = datetime.fromtimestamp(mx, tz).strftime("%Y-%m-%d") if mx else "?"
            results.append((str(db_path), cnt, s, u))
        conn.close()
    return results
...
def _sample_messages(conn: sqlite3.Connection, table: str, sender_id: int, n: int = 5) -> list[str]:
...
def _infer_my_id(conn: sqlite3.Connection, ids: list[int]) -> tuple[int, str]:
    """
    从两个候选 ID 中推断哪个是用户（my_id）。
    主信号：跨表出现频率——用户参与所有会话，在 DB 内更多 Msg_* 表中出现。
    回退：min-ID（频率相同时）。
    返回 (my_id, method) 其中 method 为 "cross-table" 或 "min-id"。
    """
    cur = conn.cursor()
    cur.execute("SELECT name FROM sqlite_master WHERE type='table' AND name LIKE 'Msg_%'")
    all_tables = [r[0] for r in cur.fetchall()]

    counts = {id_: 0 for id_ in ids}
    for t in all_tables:
        for id_ in ids:
            cur.execute(f"SELECT 1 FROM [{t}] WHERE real_sender_id = ? LIMIT 1", (id_,))
            if cur.fetchone():
                counts[id_] += 1

    if counts[ids[0]] != counts[ids[1]]:
        my_id = max(ids, key=lambda i: counts[i])
        return my_id, "cross-table"
    else:
        return min(ids), "min-id"


def infer_sender_map(db_paths: list[Path], table: str) -> list[dict]:
    """
    按库扫描 type=1 消息的 distinct real_sender_id，推断 my_id / other_id。
    主信号：跨表出现频率（用户在 DB 内更多会话表中出现）；回退：min-ID。
    ...
    for db_path in db_paths:
        ...
        if len(ids) == 2:
            my_id, method = _infer_my_id(conn, ids)
            other_id = ids[0] if ids[1] == my_id else ids[1]
