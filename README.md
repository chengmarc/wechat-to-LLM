...
微信.skill 不是导出工具。它是一个 **Claude agent 的感知层**。

其他工具的终点是一个文件。微信.skill 的起点才是一个文件——之后的查询、搜索、分析、整理，全部在 Claude 的对话里实时完成，不需要你做任何中间操作。

| | 导出工具 | 微信.skill |
|---|---|---|
| 范式 | 人操作工具 | Agent 调用工具 |
| 界面 | GUI / 命令行 | 自然语言对话 |
| 结果 | 静态文件，交给你 | Claude 的实时回答 |
| 后续 | 自己想办法 | 继续追问、分析、整理 |
| 接入 LLM | 需要额外步骤 | 原生，零距离 |

WeFlow 这类 GUI 工具做得已经很好了——如果你的目标是导出一个文件，用它。如果你的目标是**让 Claude 读懂你的微信**，用这个。
